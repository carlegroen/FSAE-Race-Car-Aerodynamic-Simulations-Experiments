...
\section{Composites}
  ...
  \end{align}
  For the biaxial carbon fiber weave used in this construction, the strength is almost $50\%$ less in the transverse direction \cite{DIAB}. The effective elastic modulus is found to be ~$\SI{60}{\giga\pascal}$. Using the beam approximation that the deflection of the rear wing functions as a beam with a core supported in both ends is found to be: \cite{styrke1}

  \begin{align}
    \dealta_\text{max} &= \frac{PL^3}{24EI} = \SI{0.11}{\milli\metre}
  \end{align}

  Which is far below the maximum allowed by a factor of $100$.
